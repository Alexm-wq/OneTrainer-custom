...
class LoadDPORejectedImageOrDummy(
    ...
    """
    Conditional rejected-image loader for mixed RLHF.

    DPO rows:
      load image_path_rejected from disk.

    Normal rows:
      do not touch image_path_rejected.
      emit zeros_like(image), only to satisfy the global image_rejected schema.

    This prevents normal rows from doing a useless second disk image load.
    The later dummy latent module prevents the useless second VAE encode.
    """
    ...
    def get_item(self, variation: int, index: int, requested_name: str = None) -> dict:
        ...
        if not is_paired:
            return {
                self.image_out_name: torch.zeros_like(image),
            }

        path = self._get_previous_item(variation, self.path_in_name, index)

        return {
            self.image_out_name: self._load_image(path),
        }
