...
class DeriveDPORejectedPath(
    ...
    - Normal rows emit image_path_rejected = image_path so the global RLHF schema
      can load/cache a dummy rejected latent. It is ignored by the normal loss.
    - DPO rows emit the actual rejected/loser image path.
    - dpo_is_paired tells the trainer which loss to use.
    """
    ...
    def start(self, variation: int):
        ...
        for index in range(self._get_previous_length(self.path_in_name)):
            ...
            # Normal concept. Use same image as dummy rejected path so LoadImage
            # succeeds, but mark row as non-DPO.
            ...
            if concept_path not in index_cache:
                index_cache[concept_path] = build_rejected_index(
                    concept_path,
                    concept.get('include_subdirectories', False),
                )
