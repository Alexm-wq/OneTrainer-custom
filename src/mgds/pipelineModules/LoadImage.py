...
from mgds.crypto import is_encrypted_file, open_source_binary
from mgds.pipelineModuleTypes.RandomAccessPipelineModule import RandomAccessPipelineModule


class LoadImage(PipelineModule, RandomAccessPipelineModule):
    def __init__(
            self,
            path_in_name: str,
            image_out_name: str,
    ...
    def get_item(self, variation: int, index: int, requested_name: str = None) -> dict:
        ...
        if ext.lower() not in self.supported_extensions:
            return {self.image_out_name: None}
        ...
        try:
            if is_encrypted_file(path):
                ...
                try:
                    image_tensor = read_image(path, self.mode, apply_exif_orientation=True).to(
            ...
            raise
        return {self.image_out_name: image_tensor}

    def _load_pillow(self, path):
        """Pillow fallback, also used for encrypted in-memory source streams."""
        with open_source_binary(path) as source:
            with Image.open(source) as image:
                image.load()
                image = ImageOps.exif_transpose(image)
                image = image.convert(self.pillow_mode)
                image_tensor = transforms.ToTensor()(image).to(device=self.pipeline.device)
        if self.dtype:
            image_tensor = image_tensor.to(dtype=self.dtype)
        return image_tensor * (self.range_max - self.range_min) + self.range_min
